...
class UtteranceEncoding():
    ...
    def generate_props(self):
        ...
        subsets += list(self.findsubsets([p for p in self.props if ' equals ' in p and not p.endswith('block')],2))
        subsets += list(self.findsubsets([p for p in self.props if ' equals ' in p and not p.endswith('block')],4))
        for block in self.blocks:
            to_remove = []
