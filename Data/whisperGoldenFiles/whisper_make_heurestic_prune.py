...
for group in range(1, 11):
    listOfDataFrames.append(pd.read_csv(f'Golden_Group_{group:02d}_CGA_W.csv'))
...
common_grounds_dataSet = pd.read_csv('../goldenFiles/NormalizedList.csv')
...
dataset = dataset.dropna(subset=['Transcript'])
#dataset = dataset.dropna(subset=['Transcript'], inplace=True)
# Now drop rows with NaN in 'Transcript' column
#dataset = dataset.dropna(subset=['Transcript'], inplace=True)
print(dataset)
...
df_extended.to_csv('whisper_preprocessedTrainingData.csv')